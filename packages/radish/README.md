...
To prevent the service worker from caching individual files, set the `no-store` cache control directive. To disable the service worker entirely, build your website with the flag `--service-worker=disabled`.
